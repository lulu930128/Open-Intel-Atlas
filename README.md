...
新增 CPI、PPI、PCE、就業薪資、GDP、失業保險申請與 FOMC 七組官方資料；schema 11、39 個指標，REST／MCP／獨立 `/macro.html` 共用 canonical capability。Macro sources 預設停用；DOL PDF 需設定 `MACRO_PDFTOTEXT_PATH`。

本次也保存必要的 Entity／Company 與 source-result 基礎依賴。隔離提交候選通過 162 項測試；發布窗口延遲、歷史全量回填與 forecast/surprise 仍待後續驗收。詳見[收尾紀錄](docs/agent-runs/macro-closeout/Progress.md)。
